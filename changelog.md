...
for the operator and FIX addresses for the owner and voting.
- Main window: the possibility of displaying private keys in the form 
of: FIX address, public key and public key hash (for diagnostics).
...
in the `protx prepare` call (added in FIX Core rc11).
...
**Added**
- Support for FIX v13 - Deterministic Masternode List (protocol 70213)
...
**Added**
- FIX daemon v12.3 support (protocol 70209/70210)
...
- Moved to the official Trezor Insight API for FIX (https://fix-bitcore1.trezor.io/api/).
...
- The connection was incorrectly shown as successful for proxy connections when nginx was running, but the FIX daemon was not.
...
- Changed the domain for "public" nodes to something more relevant: fix-masternode-tool.org.
- Changed the TCP port number for *alice* and *luna* "public" RPC nodes to 443 - the official HTTPS port.
- Added a new "public" RPC node: suzy.fix-masternode-tool.org:443.
