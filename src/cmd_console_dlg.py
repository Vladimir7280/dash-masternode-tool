...
class CmdConsoleDlg(QDialog, ui_cmd_console_dlg.Ui_CmdConsoleDlg):
    ...
    def print_help(self):
        ...
        <b>set loglevel ["module-name":"log-level",...]</b>
          ...
          Example: set loglevel all:info,fixmt.bip44_wallet:debug  
    ...
    def rpc_command(self, command: str, *args):
        if self.main_dlg.fixd_intf:
            ret = self.main_dlg.fixd_intf.rpc_call(False, True, command, *args)
            try:
            ...
            WndUtils.errorMsg('Not connected to a FIX node')
            return False
