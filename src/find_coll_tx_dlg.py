...
from fixd_intf import FixdIndexException
...
class ListCollateralTxsDlg(QDialog, ui_find_coll_tx_dlg.Ui_ListCollateralTxsDlg, wnd_utils.WndUtils):
    ...
    def display_title(self):
        if len(self.utxos):
            if self.read_only:
                msg = f'<span>Found 1000000 FIX transaction(s):</span>'
            else:
            ...
            self.lblMessage.setText('<span style="color:red"><b>Found no unspent 1000000 FIX transactions in your '
                                    'wallet</b></span>')
            self.lblMessage.setVisible(True)
