...
from fix_utils import generate_bls_privkey, generate_wif_privkey, validate_address, wif_privkey_to_address, \
    validate_wif_privkey, bls_privkey_to_pubkey
from fixd_intf import FixdInterface
...
log = logging.getLogger('fixmt.reg_masternode')


class RegMasternodeDlg(QDialog, ui_reg_masternode_dlg.Ui_RegMasternodeDlg, WndUtils):
    def __init__(self, main_dlg, config: AppConfig, fixd_intf: FixdInterface, masternode: MasternodeConfig,
        ...
        self.fixd_intf:FixdInterface = fixd_intf
        ...
        self.bip44_wallet = Bip44Wallet(self.app_config.hw_coin_name, self.main_dlg.hw_session,
                                        self.app_config.db_intf, self.fixd_intf, self.app_config.fix_network)
    ...
    def update_dynamic_labels(self):
        ...
        def get_label_text(prefix:str, key_type: str, tooltip_anchor: str, style: str):
            lbl = prefix + ' ' + \
                  {'privkey': 'private key', 'pubkey': 'public key', 'address': 'FIX address'}.get(key_type, '???')
        ...
        if self.masternode:
            ...
            if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                ...
                key_type, tooltip_anchor, placeholder_text = ('address', 'privkey', 'Enter the owner FIX address')
                style = 'hl1'
            ...
            if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                ...
                key_type, tooltip_anchor, placeholder_text = ('address', 'privkey', 'Enter the voting FIX address')
                style = 'hl1'
    ...
    def on_lblOwnerKey_linkHovered(self, link):
        if link == 'address':
            tt = 'Change input type to FIX address'
        else:
    ...
    def prepare_keys(self):
        ...
        protx_state = {}
        try:
            for protx in self.fixd_intf.protx('list', 'registered', True):
        ...
        if found_protx:
            if self.masternode.get_dmn_owner_public_address(self.app_config.fix_network) == \
            ...
            if self.masternode.get_dmn_voting_public_address(self.app_config.fix_network) == \
        ...
        if gen_owner:
            self.owner_pkey_generated =  generate_wif_privkey(self.app_config.fix_network, compressed=True)
        ...
        if self.deterministic_mns_spork_active:
            if gen_voting:
                self.voting_pkey_generated = generate_wif_privkey(self.app_config.fix_network, compressed=True)
    ...
    def on_btnGenerateOwnerKey_clicked(self, active):
        k = generate_wif_privkey(self.app_config.fix_network, compressed=True)
            ...
            if self.show_field_hinds:
                if self.edtIP.text().strip():
                    ...
                    msg = 'If don\'t set the IP address and port fields, the masternode operator will ' \
                          'have to issue a ProUpServTx transaction using FIX wallet.'
                    style = 'warning'
    ...
    def upd_payout_addr_info(self, show_invalid_data_msg: bool):
        ...
        if show_invalid_data_msg and self.payout_address_validation_err_msg:
            ...
            if self.show_field_hinds:
                msg = 'The owner\'s payout address can be set to any valid FIX address - it no longer ' \
        ...
        if show_invalid_data_msg and self.owner_key_validation_err_msg:
            ...
            if self.show_field_hinds:
                if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                    ...
                    msg = 'You can use FIX address if the related private key is stored elsewhere, eg in ' \
                          'the FIX Core wallet.<br><span class="warning">Note, that if you provide an address ' \
            ...
            if show_invalid_data_msg and self.voting_key_validation_err_msg:
                ...
                if self.show_field_hinds:
                    if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                        ...
                        msg = 'You can use FIX address if the related private key is stored elsewhere, eg in ' \
                              'the FIX Core wallet.<br><span class="warning">Note, that providing an address instead of ' \
    ...
    def get_fix_node_type(self):
        if self.rbDMTFixNodeType.isChecked():
            return NODE_TYPE_PUBLIC_RPC
        elif self.rbOwnFixNodeType.isChecked():
            return NODE_TYPE_OWN
        else:
            return None

    def upd_node_type_info(self):
        nt = self.get_fix_node_type()
        msg = ''
        if nt is None:
            msg = 'DIP-3 masternode registration involves sending a special transaction via the v0.13 FIX node ' \
                  '(eg Fix-Qt). <b>Note, that this requires incurring a certain transaction fee, as with any ' \
            ...
            msg = 'The ProRegTx transaction will be processed via the remote RPC node stored in the app configuration.' \
                  ...
                  'so use this method only if you trust the operator of that node (nodes <i>alice(luna, suzy).fix-masternode-tool.org</i> are maintained by the author of this application).<br><br>' \
                  '<b>Note 2:</b> if the operation fails (e.g. due to a lack of funds), choose the manual method ' \
                  'using your own FIX wallet.'

        elif nt == NODE_TYPE_OWN:
            msg = 'A FIX Core wallet (v0.13) with sufficient funds to cover transaction fees is required to ' \
                  'complete the next steps.'
        self.lblFixNodeTypeMessage.setText(msg)
        ...
        if self.current_step == STEP_MN_DATA:
            ...
            if self.dmn_operator_key_type == InputKeyType.PRIVATE:
                operator_message = '<b><span style="color:red">One more thing... <span></b>copy the following ' \
                                   'line to the <code>fix.conf</code> file on your masternode server ' \
                                   '(and restart <i>fixd</i>) or pass it to the masternode operator:'
            else:
                operator_message = '<b><span style="color:red">One more thing... <span></b>copy the following ' \
                                   'line to the <code>fix.conf</code> file on your masternode server, replacing ' \
    ...
    def validate_data(self):
        ...
        if not addr:
            ...
            if not validate_address(self.dmn_owner_payout_addr, self.app_config.fix_network):
        ...
        if not key:
            ...
            if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                ...
                if not validate_wif_privkey(self.dmn_owner_privkey, self.app_config.fix_network):
                    self.edtOwnerKey.setFocus()
                    self.owner_key_validation_err_msg = 'Invalid owner private key.'
                else:
                    self.dmn_owner_address = wif_privkey_to_address(self.dmn_owner_privkey, self.app_config.fix_network)
            else:
                self.dmn_owner_address = key
                self.dmn_owner_privkey = ''
                if not validate_address(self.dmn_owner_address, self.app_config.fix_network):
                    self.edtOwnerKey.setFocus()
                    self.owner_key_validation_err_msg = 'Invalid owner FIX address.'
        ...
        if self.deterministic_mns_spork_active:
            ...
            if not key:
                ...
                if self.dmn_voting_key_type == InputKeyType.PRIVATE:
                    ...
                    if not validate_wif_privkey(self.dmn_voting_privkey, self.app_config.fix_network):
                        self.edtVotingKey.setFocus()
                        self.voting_key_validation_err_msg = 'Invalid voting private key.'
                    else:
                        self.dmn_voting_address = wif_privkey_to_address(self.dmn_voting_privkey, self.app_config.fix_network)
                else:
                    self.dmn_voting_address = key
                    self.dmn_voting_privkey = ''
                    if not validate_address(self.dmn_voting_address, self.app_config.fix_network):
                        self.edtVotingKey.setFocus()
                        self.voting_key_validation_err_msg = 'Invalid voting FIX address.'
        else:
    ...
    def get_collateral_tx_address_thread(self, ctrl: CtrlObject, check_break_scanning_ext: Callable[[], bool]):
        ...
        try:
            tx = self.fixd_intf.getrawtransaction(self.dmn_collateral_tx, 1, skip_cache=True)
        except Exception as e:
        ...
        if vouts:
            if self.dmn_collateral_tx_index < len(vouts):
                ...
                if not ads or len(ads) < 0:
                    raise Exception('The collateral transaction output doesn\'t have the FIX address assigned.')
                if vout.get('valueSat') != 1000000e8:
                    raise Exception('The value of the collateral transaction output is not equal to 1000000 Fix.')
        ...
        if self.dmn_collateral_tx_address_path:
            ...
            msg = 'Looking for a BIP32 path of the FIX address related to the masternode collateral.<br>' \
    ...
    def next_step(self):
        ...
        if self.current_step == STEP_MN_DATA:
            ...
            if self.get_fix_node_type() == NODE_TYPE_PUBLIC_RPC:
                cs = STEP_AUTOMATIC_RPC_NODE
            elif self.get_fix_node_type() == NODE_TYPE_OWN:
    ...
    @pyqtSlot(bool)
    def on_rbDMTFixNodeType_toggled(self, active):
        if active:
            self.upd_node_type_info()

    @pyqtSlot(bool)
    def on_rbOwnFixNodeType_toggled(self, active):
        if active:
            self.upd_node_type_info()
    ...
    def proregtx_automatic_thread(self, ctrl):
        ...
        try:
            try:
                mn_reg_support = self.fixd_intf.checkfeaturesupport('protx_register', self.app_config.app_version)
            ...
            try:
                ...
                call_ret = self.fixd_intf.rpc_call(True, False, 'protx', *tuple(params))
            ...
            try:
                self.dmn_reg_tx_hash = self.fixd_intf.rpc_call(True, False, 'protx', 'register_submit', protx_tx,
                                                                payload_sig_str)
    ...
    def update_manual_protx_prepare_command(self):
        ...
        if addr:
            valid = validate_address(addr, self.app_config.fix_network)
            if valid:
                if self.dmn_owner_key_type == InputKeyType.PRIVATE:
                    owner_key = self.dmn_owner_privkey
                else:
    ...
    def check_tx_confirmation(self):
        try:
            tx = self.fixd_intf.getrawtransaction(self.dmn_reg_tx_hash, 1, skip_cache=True)
